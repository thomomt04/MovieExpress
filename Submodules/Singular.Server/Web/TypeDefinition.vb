...
Namespace Web.Data.JS
  ...
  Public Class TypeDefinitionList
    ...
    Private mMustRender As Boolean = True

    Public Sub SupressRendering()
      mMustRender = False
    End Sub

    Public Sub RestoreRendering()
      mMustRender = True
    End Sub
    ...
    Public Function AddTypeDefinition(ObjectType As Type, Member As Member, Optional CustomName As Object = Nothing) As Boolean

      Dim TypeName As String = GetTypeName(ObjectType, CustomName)
      If Not Me.ContainsKey(TypeName) Then
        Me.Add(TypeName, New TypeDefinition(ObjectType, TypeName, Member, mMustRender))
        Return True
      Else
        Me(TypeName).SetMustRender(mMustRender)
      End If
      Return False
    End Function
  ...
  Public Class TypeDefinition
    ...
    Friend Sub SetMustRender(MustRenderNow As Boolean)
      If Not mMustRender And MustRenderNow Then
        mMustRender = True
      End If
    End Sub
    ...
    Friend Sub RenderType(JW As Singular.Web.Utilities.JavaScriptWriter)
      If mMustRender Then
        ...
        mMember.RenderRules(JW) 'Render rules for type. If you rules arent appearing, check if you have parameterless constructor!
        mMember.RenderAdditionalTypeInfo(JW)
        ...
        mMember.RenderStaticTypeInfo(mTypeName, JW)

        JW.Write("SetupType(" & mTypeName & ",'" & mDotNetTypeName & "');", True)
        JW.RawWriteLine("")
      End If
    End Sub
